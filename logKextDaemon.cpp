...
#define SENDFREQ_PREF_KEY	CFSTR("SendFreq")
#define SENDMEG_PREF_KEY	CFSTR("SendMeg")
#define SSLURL_PREF_KEY		CFSTR("SslUrl")
#define SENTTIME_PREF_KEY	CFSTR("SentTime")
...
/*********Set up SSL**********/
	...
	if (!validKey)
	{
		prefInt = DEFAULT_SENDFREQ;
		CFNumberRef numRef = CFNumberCreate(kCFAllocatorDefault,kCFNumberIntType,&prefInt);
		CFPreferencesSetAppValue(SENDFREQ_PREF_KEY,numRef,PREF_DOMAIN);
		CFRelease(numRef);
	}
	
	CFPreferencesGetAppIntegerValue(SENDMEG_PREF_KEY,PREF_DOMAIN,&validKey);
	if (!validKey)
	{
		prefInt = DEFAULT_SENDBYTE;
		CFNumberRef numRef = CFNumberCreate(kCFAllocatorDefault,kCFNumberIntType,&prefInt);
		CFPreferencesSetAppValue(SENDMEG_PREF_KEY,numRef,PREF_DOMAIN);
		CFRelease(numRef);
	}
	
	if (!(prefStr = (CFStringRef)CFPreferencesCopyAppValue(SSLURL_PREF_KEY,PREF_DOMAIN)))
	{
		prefStr = CFSTR("");
		CFPreferencesSetAppValue(SSLURL_PREF_KEY,prefStr,PREF_DOMAIN);
	}
	else
		CFRelease(prefStr);
	
	CFPreferencesGetAppIntegerValue(SENTTIME_PREF_KEY,PREF_DOMAIN,&validKey);
	if (!validKey)
	{
		prefInt = 0;
		CFNumberRef numRef = CFNumberCreate(kCFAllocatorDefault,kCFNumberIntType,&prefInt);
		CFPreferencesSetAppValue(SENTTIME_PREF_KEY,numRef,PREF_DOMAIN);
		CFRelease(numRef);
	}
...
/*********Create Daemon Timer source**********/
	...
	CFRelease(daemonTimer); // Maybe this will cause a dangling CF object?
...
/********* Check defaults/file **********/		
	...
	if (!curPathName)		// path has been deleted
	{
		...
		logStream = CFWriteStreamCreateWithFile(kCFAllocatorDefault,url);
		CFRelease(url);
...
void updateKeymap()
{
	...
	CFURLRef url = CFURLCreateWithFileSystemPath(kCFAllocatorDefault,CFSTR(KEYMAP_PATH),kCFURLPOSIXPathStyle,false);
...
bool fileExists(CFStringRef path)
{
	struct stat fileStat;
	if (stat(CFStringGetCStringPtr(path,CFStringGetFastestEncoding(path)),&fileStat))
		return false;
	return true;
}

bool outOfSpace(CFStringRef path)
{
	Boolean			validKey;
	unsigned int	minMeg;
	...
	{
		minMeg = DEFAULT_MEG;
		CFNumberRef n = CFNumberCreate(kCFAllocatorDefault,kCFNumberIntType,&minMeg);
		CFPreferencesSetAppValue(MINMEG_PREF_KEY,n,PREF_DOMAIN);
		CFRelease(n);
	}

	if (statfs(CFStringGetCStringPtr(path,CFStringGetFastestEncoding(path)),&fileSys))
		return false;
...
void LoginLogoutCallBackFunction(SCDynamicStoreRef store, CFArrayRef changedKeys, void * info)
{
    ...
    {
		CFStringRef sRef = CFStringCreateWithFormat(NULL, NULL, CFSTR("User '%@' has logged in"), consoleUserName);
		stamp_file(sRef);
		CFRelease(sRef);
